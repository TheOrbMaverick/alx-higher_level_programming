...
SET @dexter_genre_ids := (SELECT GROUP_CONCAT(tv_show_genres.genre_id) FROM tv_show_genres JOIN tv_shows ON shows.id = tv_show_genres.show_id WHERE tv_shows.title = 'Dexter');
